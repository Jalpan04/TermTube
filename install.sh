...
install_dependencies() {
    ...
    case $DISTRO in
        arch|manjaro)
            ...
            sudo pacman -Syu --noconfirm yt-dlp jq fzf ffmpeg
            ;;
        debian|ubuntu|kali|linuxmint)
            echo "Detected Debian/Ubuntu based system."
            sudo apt update
            sudo apt install -y yt-dlp jq fzf ffmpeg
            ;;
        macos)
            echo "Detected macOS."
            ...
            brew install yt-dlp jq fzf ffmpeg
            ;;
        *)
            echo -e "${RED}Unsupported distribution: $DISTRO${NC}"
            echo "Please install 'yt-dlp', 'jq', 'fzf', and 'ffmpeg' manually."
            ;;
    esac
}
